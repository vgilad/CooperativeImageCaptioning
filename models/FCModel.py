...
class FCModel(nn.Module):
    ...
    def forward(self, fc_feats, att_feats, att_masks, seq, masks):
        ...
        output = torch.cat(
            [_.unsqueeze(1) for _ in outputs[1:]], 1).contiguous()
    ...
    def sample_beam(self, fc_feats, att_feats, att_masks, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(self.seq_length + 2):
                if t == 0:
                    xt = self.new_img_embed(
                        fc_feats[k:k+1],
                        fc_feats_d.chunk(batch_size)[k]).expand(
                    ...
                    for vix in range(beam_size):
                        ...
                        if t > 2:
                            ...
                            beam_seq_logprobs[:t-2, vix] = \
                                beam_seq_logprobs_prev[:, v['q']]

                        # Rearrange recurrent states
                        for state_ix in range(len(new_state)):
                            # Copy over state in previous beam q to
                            # new beam at vix.
                        ...
                        if v['c'] == 0 or t == self.seq_length + 1:
                            # END token special case here, or we
                            # reached the end. add the beam to a set of
                            # done beams
            ...
            self.done_beams[k] = sorted(
                self.done_beams[k], key=lambda x: -x['p'])
            # The first beam has highest cumulative score
            seq[:, k] = self.done_beams[k][0]['seq']
            seqLogprobs[:, k] = self.done_beams[k][0]['logps']
        # Return the samples and their log likelihoods
        return Variable(seq.transpose(0, 1)), Variable(
    ...
    def sample(self, fc_feats, att_feats, att_masks, opt={}):
        ...
        for t in range(self.seq_length + 2):
            if t == 0:
                ...
                if t == 1: # input <bos>
                    it = Variable(fc_feats.data.new(batch_size).long().fill_(
                        self.vocab_size + 1))
                elif sample_max == 1:
