...
def main(params):
  ...
  for i,img in enumerate(imgs):
    # load the image
    if params['dataset'] in ['flickr8k', 'flickr30k']:
      I = skimage.io.imread(
    ...
    # write to pkl
    if params['dataset'] in ['flickr8k', 'flickr30k']:
      np.save(os.path.join(dir_fc, str(img['filename'].split('.')[0])),
                           tmp_fc.data.cpu().float().numpy())
      np.savez_compressed(os.path.join(
        dir_att, str(img['filename'].split('.')[0])),
...
if __name__ == "__main__":
  ...
  parser.add_argument('--images_root', default='',
                      help='root location in which images are stored, '
                           'to be prepended to file_path in input json')
  ...
  parser.add_argument('--model_root', default='./misc/', type=str,
                      help='model root')
  parser.add_argument('--dataset', default='flickr30k', type=str,
                      help='coco / flickr8k / flickr30k')
