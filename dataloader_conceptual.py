...
class DataLoader_conceptual(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.dataset = getattr(opt, 'dataset', 'coco')
        # feature related options
        self.use_fc = getattr(opt, 'use_fc', True)
        # TODO - in phase 1 training don't load att features, a change need
        # to be done in train.py
        self.use_att = getattr(opt, 'use_att', True)  # self.use_att = True
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            if self.dataset in ['flickr8k', 'flickr30k']:
                image_id = self.info['images'][ix][
                    'file_path'].split('/')[1].split('.')[0]
                att_feat = self.att_loader.get(str(image_id))
            else:  # coco
        ...
        if self.use_fc:
            if self.dataset in ['flickr8k', 'flickr30k']:
                image_id = self.info['images'][ix][
                    'file_path'].split('/')[1].split('.')[0]
                fc_feat = self.fc_loader.get(str(image_id))
            else:  # coco\ conceptual
...
class BlobFetcher():
    ...
    def get(self):
        ...
        tmp_tupple = self.split_loader.next()
        tmp = list(tmp_tupple)
        if wrapped:
            self.reset()

        assert tmp[-1] == ix, "ix not equal"
        #print(f"tmp type is {type(tmp)}, wrapped type is {type(wrapped)}")
        return tmp + [wrapped]
